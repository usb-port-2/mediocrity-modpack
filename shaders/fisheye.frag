...
{
	vec2 p = getCamPos((openfl_TextureCoordv * openfl_TextureSize.xy) / openfl_TextureSize.x);//normalized coords with some cheat
	                                                         //(assume 1:1 prop)
	float prop = openfl_TextureSize.x / openfl_TextureSize.y;//screen proroption
	vec2 m = vec2(0.5, 0.5 / prop);//center coordsa
